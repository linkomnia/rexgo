...
/*
Package rexpro1 implements the RexPro version 1 protocol.
...
type Version byte // must be 1
type SerializerType byte
type MsgType byte

const (
	SerializerTypeMsgPack SerializerType = iota
	SerializerTypeJSON
)
...
func (client *Client) writeMessage(typ MsgType, session [16]byte, seq uint64, meta MetaMap, args []interface{}) (err error) {
	...
	h := &MsgHeader{1, SerializerTypeMsgPack, [4]byte{0, 0, 0, 0}, typ, uint32(len(msgBytes))}
...
func (client *Client) input() {
	...
	if err != io.EOF && !closing {
		log.Print("rexpro1: client protocol error: ", err)
	}
}
...
func (call *Call) done() {
	...
	default:
		...
		log.Println("rexpro1: discarding Call reply due to insufficient Done chan capacity")
	}
}
...
func (client *Client) Go(session [16]byte, typ MsgType, meta MetaMap, args []interface{}, done chan *Call) *Call {
	...
	if done == nil {
		...
		if cap(done) == 0 {
			log.Panic("rexpro1: done channel is unbuffered")
		}
	}
	call.Done = done
	client.send(call)
