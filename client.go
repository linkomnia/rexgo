...
func (rc *rxConn) closeClientLocked() func() error {
	...
	defer rc.Unlock()
	if rc.closed {
		return func() error { return errors.New("rexgo: duplicate rxConn close") }
	}
	rc.closed = true
...
func (rc *rxConn) Close() error {
	...
	if rc.closed {
		rc.Unlock()
		return errors.New("rexgo: duplicate rxConn close")
	}
	rc.closed = true
...
func (client *Client) putConn(rc *rxConn, err error) {
	...
	if !rc.inUse {
		...
		panic("rexgo: connection returned that was never out")
	}
	if debugGetPut {
